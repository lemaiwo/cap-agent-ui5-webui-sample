...
export default defineConfig({
  ...
  webServer: {
    // dev:scripted, not dev: the mock LLM that ships with @cap-js/agents ignores
    // the prompt and can only ever call the read tool, so the approval flow -
    // the most interesting thing to test - is unreachable without the scripted
    // stand-in.
    command: "npm run dev:scripted",
    url: "http://localhost:4004/a2a/support/.well-known/agent-card.json",
    // false, not !process.env.CI: reusing a server started some other way means
    // reusing one without AGENT_LLM=scripted, and the HITL tests then fail with
    // "Approve never appeared" for no visible reason.
